...
import type { LeadsBySourceData } from '@/lib/dashboard/types'
...
export function LeadsSourceDonut({ data, loading }: LeadsSourceDonutProps) {
  const t = useTranslations('Dashboard.leadsBySource')
  return (
...
// ------------------------------------------------------------
// Same hand-rolled SVG ring as PipelineDonut (see that file for the
// rationale) — no vendored Tremor donut exists in this codebase, and
// the two donuts' data shapes (currency total vs. plain count) are
// different enough that sharing one generic component would need as
// much branching as just having two small ones.
// ------------------------------------------------------------
function Donut({ data }: { data: LeadsBySourceData }) {
  const t = useTranslations('Dashboard.leadsBySource')
  const size = 200
  const r = 80
  const ringWidth = 18
  ...
  const minFrac = 0.02
  const rawShares = data.sources.map((s) => s.count / totalRaw)
