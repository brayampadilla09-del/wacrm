...
export interface Contact {
  ...
  user_id: string;
  account_id: string;
  /** Which WhatsApp number this contact came in on (migration 039) —
   *  the same phone can be an independent contact per channel. */
  channel_id: string;
  phone: string;
...
export interface Conversation {
  ...
  user_id: string;
  contact_id: string;
  /** Which WhatsApp number this conversation came in on (migration 039). */
  channel_id: string;
...
export interface WhatsAppConfig {
  id: string;
  user_id: string;
  /** Display name for this channel (migration 039) — e.g. "Bimi", "Asesor". */
  label: string;
  /** 'bot' runs flows/automations/AI auto-reply; 'human' is a plain
   *  manual inbox with no automated responders. */
  kind: 'bot' | 'human';
  /** The channel that loads first / that legacy single-channel call
   *  sites fall back to. Exactly one per account. */
  is_default: boolean;
  /** Who to push-notify on every inbound message on this channel.
   *  NULL means "notify every agent on the account". */
...
export interface Deal {
  id: string;
  user_id: string;
  /** Which WhatsApp channel this deal's contact belongs to (migration 039). */
  channel_id: string;
  pipeline_id: string;
  stage_id: string;
  /**
...
export interface Broadcast {
  id: string;
  user_id: string;
  /** Which WhatsApp channel this broadcast sends from (migration 039). */
  channel_id: string;
  name: string;
  template_name: string;
...
export interface Automation {
  ...
  account_id: string;
  /** Which WhatsApp number this automation runs on (migration 039). */
  channel_id: string;
